...
A local, projector-ready video wall for the Black Forest Labs and Nous Research event.
...
2. Add the early access API key to `BFL_API_KEY`.
3. Add the tested model version to `BFL_MODEL_VERSION` when one is available.
...
If there is no manifest entry, the film is credited to `BFL × Nous`. Edit `data/seed-videos/manifest.json` to add a creator, prompt, and duration:
...
The console at the bottom is keyboard only. It walks each guest through three
stages: name, film, length.

- Typing anywhere lands in the current field. Guests just walk up and type.
...
The default endpoint is `flux-3-preview-optimized` with eight concurrent jobs. Change the endpoint and concurrency in `.env.local` if the event uses `flux-3-preview-high`.
